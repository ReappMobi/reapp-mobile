...
import { getProjectDonations } from 'src/services/donations';
import type { Donation } from 'src/types/IDonation';
...
const MyDonationsInstitutionProjectsPage = () => {
  ...
  const fetchDonations = async (
    ...
    try {
      const result = await getProjectDonations(targetPage, period, token);
      if (isRefresh) {
        setData(result);
      } else {
        setData((prev) => ({
