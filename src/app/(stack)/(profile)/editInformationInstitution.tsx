...
import { editInstitutionInformation } from 'src/services/user';
import { z } from 'zod';

const editInformationInstitutionFormSchema = z.object({
  name: z
    .string()
...
type editInformationFormData = z.infer<
  typeof editInformationInstitutionFormSchema
>;

export default function EditInformationInstitution() {
  const auth = useAuth();
  ...
  const {
    ...
    resolver: zodResolver(editInformationInstitutionFormSchema),
  });
  ...
  const onSubmit = async (data: any) => {
    ...
    const res = await editInstitutionInformation(dataReq);
    setLoading(false);
    if (res.error) {
